...
class Admin_Class {
    ...
    public function admin_login_check( $data ) {
        ...
        try {
            ...
            if ( $stmt->rowCount() > 0 ) {
                ...
                $_SESSION[ 'phonenumber' ] = $userRow[ 'phone_number' ];
    ...
    public function add_new_user( $data ) {
        ...
        $user_phonenumber = $this->test_form_input_data( $data[ 'em_phonenumber' ] );
        ...
        try {
            ...
            if ( $total_email != 0 && $total_username != 0 ) {
                ...
                $add_user = $this->db->prepare( 'INSERT INTO tbl_admin (fullname, username, email, password, temp_password, user_role,phone_number) VALUES (:x, :y, :z, :a, :b, :c,:p) ' );
                ...
                $add_user->bindparam( ':p', $user_phonenumber );
    ...
    public function update_user_data( $data, $id ) {
        ...
        $user_phonenumber = $this->test_form_input_data( $data[ 'em_phonenumber' ] );
        try {
            $update_user = $this->db->prepare( 'UPDATE tbl_admin SET fullname = :x, username = :y, email = :z, phone_number =:p WHERE user_id = :id ' );

            $update_user->bindparam( ':x', $user_fullname );
            $update_user->bindparam( ':y', $user_username );
            $update_user->bindparam( ':z', $user_email );
            $update_user->bindparam( ':p', $user_phonenumber );
    ...
    public function add_new_task( $data ) {
        ...
        try {
            ...
            $stmt = $this->db->prepare( 'SELECT * FROM tbl_admin WHERE user_id=:id LIMIT 1' );
            $stmt->execute( array( ':id'=>$assign_to ) );
            $userRow = $stmt->fetch( PDO::FETCH_ASSOC );
            ...
            $message = 'You have a new task. Please check your task list';

            $msgsentstatus = $this->SendSms( $phonenumber, $message );
    ...
    public function SendSms( $to, $message ) {
        ...
        if ( empty( $to ) && empty( $message ) ) {
            echo 'Phone number and message can not be empty';
            return false;
        }
        ...
        if ( $data !== null && isset( $data[ 'code' ] ) && $data[ 'code' ] == 'ok' ) {
